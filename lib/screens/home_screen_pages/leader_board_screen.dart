...
class _LeaderBoardScreenState extends State<LeaderBoardScreen> {
  ...
  int currentIndex = 0;
  List<LeaderBoardModel> leaderListAllTimes = [];
  List<LeaderBoardModel> allListWithoutTopThree = [];
  int network = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                  ...
                  children: List.generate(days.length, (index) {
                    ...
                    return InkWell(
                      onTap: (){
                        ...
                        if(currentIndex == 0){
                          context.read<LeaderBoardProvider>().getTwentyFourHourList(context);
                          // leaderListAllTimes = context.read<LeaderBoardProvider>().leaderBoardListTwentyFourHour;
                          // leaderListAllTimes = context.read<LeaderBoardProvider>().leaderBoardList;
                          // print(leaderListAllTimes);
                          initListOtherThanTopThree();
                        }else if(currentIndex == 1){
                          context.read<LeaderBoardProvider>().getOneWeekLeaderBoardList(context);
                          // leaderListAllTimes = context.read<LeaderBoardProvider>().leaderBoardListOneWeek;
                          // leaderListAllTimes = context.read<LeaderBoardProvider>().leaderBoardList;
                          initListOtherThanTopThree();
                        }else if(currentIndex == 2){
                          context.read<LeaderBoardProvider>().getAllTimeLeaderBoardList(context);
                          // leaderListAllTimes = context.read<LeaderBoardProvider>().leaderBoardListAllTimes;
                          // leaderListAllTimes = context.read<LeaderBoardProvider>().leaderBoardList;
                          // initListOtherThanTopThree();
                        }
                      },
      ...
      bottomNavigationBar: Consumer<UserInformationProvider>(
              ...
              child: Row(
                ...
                children: [
                  Row(
                    children: [
                      currentIndex == 0 ?
                      Text(value.userModel!.twentyFourHourRank.toString()) :
                      currentIndex == 1 ? Text(value.userModel!.weeklyRank.toString()) :
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Text(name,style: TextStyle(fontSize: Dimensions.height10,color: Colors.white),),
        Container(
        ...
        Row(
          children: [
            Image.asset('assets/icons/leader_board_icons/star.png',height: Dimensions.height20,width: Dimensions.height20,),
            const SizedBox(width: 10,),
            Text(points,style: TextStyle(fontSize: Dimensions.height15,color: Colors.white),)
          ],
        ),
      ],
    );
